...
final class OverdrivePlayer: NSObject, Player {
    ...
    var isDrmOk: Bool {
        didSet(oldValue) {
            if !oldValue {
                pause()
                notifyDelegatesOfPlaybackFailureFor(chapter: self.chapterAtCurrentCursor, NSError(domain: OverdrivePlayerDomain, code: 4, userInfo: nil))
                unload()
            }
        }
    }
    ...
    func play()
    {
        ...
        if !isDrmOk {
            ...
            let error = NSError(domain: OverdrivePlayerDomain, code: 4, userInfo: nil)
        ...
        case .unknown:
            ...
            if self.avQueuePlayer.currentItem == nil {
                ATLog(.debug, "AV Player current item is nil")
                if let cursorItemDownloadTask = self.cursor.currentElement.downloadTask as? OverdriveDownloadTask {
                    switch cursorItemDownloadTask.assetFileStatus() {
                    case .saved(let savedURL):
                        ATLog(.debug, "File status - saved")
                        let item = AVPlayerItem(url: savedURL)
                        if self.avQueuePlayer.canInsert(item, after: nil) {
                            ATLog(.debug, "Inserting item into av player")
                            self.avQueuePlayer.insert(item, after: nil)
                        }
                    case .missing(_):
                        ATLog(.debug, "File status - missing")
                        self.rebuildOnFinishedDownload(task: cursorItemDownloadTask)
                    default:
                        ATLog(.debug, "File status - unknown")
                        break
                    }
                }
                if let cursorItemDownloadTask = self.cursor.currentElement.downloadTask as? OverdriveDownloadTask {
        ...
        case .failed:
            ...
            let error = NSError(domain: OverdrivePlayerDomain, code: 0, userInfo: nil)
            self.notifyDelegatesOfPlaybackFailureFor(chapter: self.chapterAtCurrentCursor, error)
            break
        }
    }
    ...
    func pause()
    {
        if self.isPlaying {
            ...
            NotificationCenter.default.removeObserver(self, name: ODTaskCompleteNotification, object: nil)
    ...
    func playAtLocation(_ newLocation: ChapterLocation)
    {
        ...
        let newItemDownloadTask = newPlayhead.cursor.currentElement.downloadTask as? OverdriveDownloadTask
        ...
        case .missing(_):
            ...
            let error = NSError(domain: OverdrivePlayerDomain, code: 1, userInfo: nil)
            self.notifyDelegatesOfPlaybackFailureFor(chapter: newLocation, error)
            return
        case .unknown:
            let error = NSError(domain: OverdrivePlayerDomain, code: 0, userInfo: nil)
                ...
                if let cursor = self.cursorQueuedToPlay {
                    ...
                    self.buildNewPlayerQueue(atCursor: cursor) { success in
                        if success {
                            ...
                            let error = NSError(domain: OverdrivePlayerDomain, code: 2, userInfo: nil)
                            self.notifyDelegatesOfPlaybackFailureFor(chapter: self.chapterAtCurrentCursor, error)
                        }
                    }
                }
    ...
    private var overdrivePlayerContext = 0
    ...
    private func buildPlayerItems(fromCursor cursor: Cursor<SpineElement>?) -> [AVPlayerItem]
    {
        ...
        while (cursor != nil) {
            guard let downloadTask = cursor!.currentElement.downloadTask as? OverdriveDownloadTask else {
                cursor = nil
                continue
            }
    ...
    {
        if let cursorItemDownloadTask = cursor.currentElement.downloadTask as? OverdriveDownloadTask {
            switch cursorItemDownloadTask.assetFileStatus() {
            case .saved(_):
    ...
    fileprivate func rebuildOnFinishedDownload(task: OverdriveDownloadTask)
    {
        ATLog(.debug, "Added observer for missing download task.")
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(self.ODDownloadTaskFinished),
                                               name: ODTaskCompleteNotification,
                                               object: task)
    }
    
    @objc func ODDownloadTaskFinished()
    {
...
extension OverdrivePlayer{
    override func observeValue(forKeyPath keyPath: String?,
                               of object: Any?,
                               change: [NSKeyValueChangeKey : Any]?,
                               context: UnsafeMutableRawPointer?)
    {
        guard context == &overdrivePlayerContext else {
    ...
    fileprivate func addPlayerObservers() {
        self.avQueuePlayer.addObserver(self,
                                       ...
                                       options: [.old, .new],
                                       context: &overdrivePlayerContext)
